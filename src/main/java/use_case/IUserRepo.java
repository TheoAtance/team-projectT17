...
/**
 * The Repository for accessing and persisting the User Entity in the database (Firebase
 * Firestore).
 */
public interface IUserRepo {
  ...
  /**
   * Checks if a User Entity with the given UID already exists in the Firestore database. This will
   * be useful for determining if a Google sign-in is a "login" or a "first-time registration."
   *
   * @param uid The unique Firebase ID to check.
  ...
  /**
   * Get all user documents from firebase and use each user's data to make a user obj then map the
   * user's uid to its respective user obj then save this in-memory map as a field for easy access
   * to user data.
   */
