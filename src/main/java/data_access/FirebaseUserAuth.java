...
/**
 * Rigorous implementation of IAuthGateway using the public Firebase Authentication REST API. This
 * requires the Firebase Web API Key for secure client-side operations.
 * <p>
 * NOTE: This version uses the Jackson ObjectMapper for reliable JSON parsing. MUST add Jackson
 * dependencies (e.g., jackson-databind) to the project.
 */
public class FirebaseUserAuth implements IAuthGateway {
  ...
  public FirebaseUserAuth() {
    ...
    System.out.println(
        "   webApiKey length: " + (this.webApiKey != null ? this.webApiKey.length() : "NULL"));
  }
   ...
   */
  private String executeAuthRequest(String baseUrl, String payload, String action)
      throws AuthFailureException {
    ...
    System.out.println(
        "   this.webApiKey length = " + (this.webApiKey != null ? this.webApiKey.length()
            : "NULL"));
    ...
    System.out.println(
        "   API Key (first 20 chars): " + webApiKey.substring(0, Math.min(20, webApiKey.length()))
            + "...");
  ...
  /**
   * Initiates Google Sign-in. In a desktop environment, this requires the user to obtain a Google
   * Access Token via a browser flow and manually provide it. The token is then exchanged for a
   * Firebase UID and ID token.
   *
  ...
  public GoogleAuthResult loginWithGoogle() throws AuthFailureException, IOException {
    ...
    try {
      ...
      throw new AuthFailureException(
          "Google Sign-in failed due to network error: " + e.getMessage());
    }
  ...
  /**
   * Handles the Swing UI interaction to retrieve the Google **Authorization Code**, then exchanges
   * that code for the required **Access Token**. * This method is the core of the Google login
   * flow.
   *
  ...
  private String getGoogleAccessTokenFromCode() throws AuthFailureException, IOException {
    if (!Desktop.isDesktopSupported()) {
      JOptionPane.showMessageDialog(null,
          "Desktop operations not supported. Cannot launch browser.",
    ...
    try {
      ...
      throw new AuthFailureException(
          "Failed to exchange authorization code for access token. Server response: "
              + response.body());

    } catch (IOException | InterruptedException e) {
      throw new AuthFailureException(
          "Google token exchange failed due to network error: " + e.getMessage());
    }
  }

  /**
   * Clears the current user session (signs out the user). Since this implementation uses the REST
   * API (stateless), 'logout' is a NO-OP in the data access layer. The calling layer (use
   * case/controller) is responsible for clearing any locally stored Firebase ID Tokens or session
   * data.
   */
  @Override
  public void logout() {
    ...
    System.out.println(
        "Logout: Data Access Layer executed. Local session cleanup is required upstream.");
  }
  ...
  private String extractUidFromJson(String json) {
    ...
    throw new AuthFailureException(
        "Authentication successful, but could not parse user ID from response.");
  }
