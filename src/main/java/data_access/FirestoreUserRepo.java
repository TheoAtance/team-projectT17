...
/**
 * Firestore implementation of the IUserRepo interface. This class handles all database operations
 * for User entities using Firebase Firestore.
 * <p>
 * Firestore is a NoSQL document database where data is stored in collections of documents. Each
 * User is stored as a document in the "users" collection, with the Firebase Auth UID as the
 * document ID for efficient O(1) lookups.
 * <p>
 * All operations are asynchronous but use ApiFuture.get() to block until completion, simplifying
 * the programming model at the cost of blocking the calling thread.
 *
 * @see IUserRepo
 * @see User
...
public class FirestoreUserRepo implements IUserRepo {
  ...
  /**
   * The name of the Firestore collection where user documents are stored. Collection structure:
   * users/{uid}/
   */
  ...
  /**
   * Constructs a new FirestoreUserRepo and initializes the Firestore database connection. The
   * Firestore instance is obtained from the FirebaseService singleton.
   */
   ...
   * <p>
   * This method performs type-safe extraction of data from the Firestore document, handling
   * potential type mismatches and missing fields gracefully. It uses the UserFactory to construct
   * the User entity with proper validation.
   * <p>
   * Firestore Document Structure:
   * <p>
   * users/{uid}/ ├── email: string ├── nickname: string ├── language: string └──
   ...
   * <p>
   * This method performs a direct document lookup using the UID as the document ID, resulting in
   * O(1) time complexity. The operation is asynchronous but blocks until the document is retrieved
   * from Firestore.
   *
   ...
   * can store, then performs a set() operation which acts as an "upsert" (insert or update). The
   * operation is asynchronous but blocks until the write is confirmed by Firestore.</p>
   *
  ...
  public void save(User user) throws PersistenceException {
    try {
      ...
      throw new PersistenceException(
          "Failed to save user " + user.getUid() + ": " + e.getMessage());
    }
  }
    ...
    try {
      ...
      for (QueryDocumentSnapshot document : documents) {
        try {
          ...
          System.err.println(
              "Skipping malformed user document " + document.getId() + ": " + e.getMessage());
        }
      }
   ...
   * (insert or update), so this method simply delegates to save(). If you need to update only
   * specific fields without replacing the entire document, use Firestore's update() method
   * instead.</p>
   *
   * <p>Usage Example:</p>
   * <pre>
   ...
   * It does NOT retrieve the document data, only checks for existence. However, Firestore still
   * charges a read operation for this check.</p>
   *
   ...
   * Note that this does NOT delete the user's Firebase Authentication account - that must be done
   * separately through IAuthGateway.</p>
   *
