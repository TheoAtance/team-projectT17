...
/**
 * The View for the Favorites Use Case. Uses RestaurantPanel components to display favorite
 * restaurants.
 */
public class FavoritesPageView extends JFrame implements PropertyChangeListener {
  ...
  private void updateRestaurantsDisplay(FavoritesState state) {
    ...
    if (state.getRestaurants().isEmpty()) {
      ...
      setTitle(
          "Favorite Restaurants (" + state.getRestaurants().size() + ") - " + state.getUsername());
      restaurantsContainer.setLayout(new GridLayout(0, 3, 20, 20));
      restaurantsContainer.setBackground(new Color(249, 250, 251));
  ...
  private RestaurantPanel createRestaurantPanel(
      FavoritesState.RestaurantDisplayData restaurantData) {
  ...
  private RestaurantPanel.RestaurantDisplayData convertToRestaurantPanelData(
      FavoritesState.RestaurantDisplayData data) {
